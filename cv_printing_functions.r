...
create_CV_object <- function(data_location,
  ...
  if (is_google_sheets_location) {
    ...
    cv$interests <- read_gsheet(sheet_id = "interests")
    cv$awards_services <- read_gsheet(sheet_id = "awards_services")
  } else {
    ...
    cv$interests <- readr::read_csv(paste0(data_location, "interests.csv"), skip = 1)
  }
  ...
  if ("description_md" %in% colnames(cv$entries_data)) {
    ...
    cv$entries_data <- cv$entries_data |>
      ...
      dplyr::mutate(
        description_bullets = ifelse(!is.na(description_bullets) | 
                                description_bullets != "", 
...
\n\n\n"
  ...
  for (i in 1:nrow(section_data)) {
    for (col in c("description_bullets")) {
...
print_contact_info <- function(cv) {
  cv$contact_info |>
    dplyr::mutate(
      contact = ifelse(
        is.na(url) | url == "" | url == "NA",
        contact,  # If no URL, just show contact text
        glue::glue("<a href='{url}' target='_blank'>{contact}</a>")  # Otherwise, clickable link
      )
    ) |>
...
print_interests <- function(cv) {
  glue::glue_data(
    cv$interests,
    "- <i class='fa fa-{icon}'></i> {interest}"
  ) |> print()
  
  invisible(cv)
}

print_awards_services <- function(cv) {
  glue::glue_data(
    cv$awards_services,
